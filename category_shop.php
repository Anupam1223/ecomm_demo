<?php

include "config.php";

$page = basename($_SERVER['PHP_SELF']);
switch($page){
  case "category_shop.php":
    if($shop_id=$_GET['sid']){
      $sql_title = "SELECT * FROM shop where shop = $shop_id";
      $result_title = oci_parse($conn,$sql_title) or die("Sorry ! Title might not be available right now.");
      oci_execute($result_title);
      $row_title = oci_fetch_assoc($result_title);
      $page_title = $row_title['SHOPE_NAME'];
    }else{
      $page_title = "Product | Category";
    }
    break;
}
?>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link crossorigin="anonymous" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/css/all.min.css"
        integrity="sha512-iBBXm8fW90+nuLcSKlbmrPcLa0OT92xO1BIsZ+ywDWZCvqsWgccV3gFoRBv0z+8dLJgyAHIhR35VZc2oM/gI1w=="
        referrerpolicy="no-referrer" rel="stylesheet" />
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css"
        integrity="sha384-MCw98/SFnGE8fJT3GXwEOngsV7Zt27NXFoaoApmYm81iuXoPkFOJwJ8ERdknLPMO" crossorigin="anonymous">
    <link rel="stylesheet" href="style/style.css">
    <link rel="shortcut icon" href="images/titlebar.png" type="image/png">
    <title><?php echo $page_title ?></title>
</head>

<body>

    <div id="navbar">
        <?php include "navbar.php" ;
              include "config.php";?></div>
    <div class="container">
        <?php include "category.php" ?>
    </div>

    <?php 

    $shop_id=$_GET['sid'];
    
    $sql_shop = "SELECT * FROM shop where shop = $shop_id";

    $shop_result = oci_parse($conn,$sql_shop);
    oci_execute($shop_result);

    $count = oci_fetch_assoc($shop_result);

    if($count > 0){
    
    ?>
    <div class="item-header my-md-3 mt-2 bg-white">
        <div class="container d-sm-flex">
            <?php while($shop_row = oci_fetch_assoc($shop_result)){ ?>
            <h4 class="py-2 text-center my-auto"><?php echo $shop_row['SHOPE_NAME'] ?></h4>
            <?php } ?>
        </div>
    </div>
    <?php } ?>

    <main class="all-items">
        <div class="container">

            <div class="row justify-content-center justify-content-md-start">
                <?php
                      
			            $sql = "SELECT * FROM product WHERE shop = $shop_id";
			            $result = oci_parse($conn,$sql);
                        oci_execute($result);
                            
				            while($row = oci_fetch_assoc($result)){ 
                                 
                                $pid= $row['PRODUCT_ID'];
                                $sql_discount = "SELECT offer_amount FROM product_offer WHERE product_id =$pid ";
                                $discount_result = oci_parse($conn,$sql_discount);
                                oci_execute($discount_result);
                                $product_offer_amount = oci_fetch_assoc($discount_result);
                                ?>

                <div class="col-md-4 col-lg-3 col-sm-6 col-12 my-2">

                    <div class="card shadow">
                        <img class="card-img-top img-fluid border-bottom"
                            src="images/<?php echo $row['PRODUCT_IMAGE']; ?>" alt="">
                            <div class="card-body">
                                    <h5 class="product-name card-title text-center"><?php echo $row['PRODUCT_NAME']; ?>
                                    </h5>
                                    <p class="text-center">
                                    
                                        <?php 
                                            $final_price = $row['PRODUCT_PRICE'];
                                            if(!empty($product_offer_amount['OFFER_AMOUNT'])){
                                                $final_price = $row['PRODUCT_PRICE']-$product_offer_amount['OFFER_AMOUNT'];
                                            }
                                            echo '£'.$final_price; 
                                        ?>

                                        <?php if(!empty($product_offer_amount)) {?>
                                        <small class="pl-3 text-muted">
                                            <?php echo '£'.$product_offer_amount['OFFER_AMOUNT'].' off' ?></small>
                                    </p>
                                    <?php }?>
                                    <p class="mr-2 text-center"><?php echo $row['RATING']; ?>
                                        <i class="fas fa-star fa-md text-warning"></i>
                                    </p>
                                    <a href="products.php?pid=<?php echo $row['PRODUCT_ID']; ?>"
                                        class="btn btn-primary btn-sm w-100 mb-2">View Details</a>
                                    <a href="" class="add-to-cart btn btn-warning w-100 btn-sm"
                                        data-id="<?php echo $row['PRODUCT_ID']; ?>"><i class="fas fa-shopping-cart pr-2">
                                        </i>Add To Cart</a>
                                </div>
                    </div>
                </div>
                <?php }?>
            </div>
        </div>
        </div>
    </main>

    <div class="footer-bg pt-2 mt-3">
            <?php include "footer.php" ?>
    </div>

    <?php include "script.php" ?>
    <script src="http://code.jquery.com/jquery-3.4.0.min.js"
        integrity="sha256-BJeo0qm959uMBGb65z40ejJYGSgR7REI4+CW1fNKwOg=" crossorigin="anonymous"></script>
    <script>
    $(document).ready(function() {

        $('.add-to-cart').click(function(e) {
            e.preventDefault();
            var p_id = $(this).attr('data-id');
            $.ajax({
                url: 'actions.php',
                method: 'POST',
                data: {
                    addCart: p_id
                },
                success: function(data) {
                    $('#navbar').load('index.php #navbar', function() {
                        setTimeout(10);
                    });
                    setTimeout(8000);
                }
            });
        });
    });
    </script>
</body>

</html>